...
# Check requirements - Now includes aria2c
check_requirements() {
    local missing_tools=()
...
fetch_wayback() {
    ...
    # Run both queries in parallel
    {
        ...
        curl -s --retry $MAX_RETRIES --retry-delay 2 "http://index.commoncrawl.org/CC-MAIN-latest-index?url=*.${domain}/*&output=json" | jq -r '.url' > "${output_dir}/endpoints/wayback_urls_2.txt" &
        wait
    } 
    ...
    print_status "Found $(wc -l < "${output_dir}/endpoints/wayback_urls.txt") URLs from archives"
}
...
fetch_subdomains() {
    ...
    {
        # Subfinder with increased threads
        if command -v subfinder &> /dev/null; then
            subfinder -d "$domain" -t 100 -silent > "${temp_dir}/subfinder.txt" &
        fi
        
        # Amass with optimized settings
        if command -v amass &> /dev/null; then
            amass enum -passive -d "$domain" -timeout 5 > "${temp_dir}/amass.txt" &
        fi
        
        # Rapid7 FDNS
        curl -s "https://dnsdumpster.com/api/v2/domain/${domain}" > "${temp_dir}/dnsdumpster.txt" &
        
        # crt.sh with optimized query
        curl -s --retry $MAX_RETRIES "https://crt.sh/?q=${domain}&output=json" | jq -r '.[].name_value' > "${temp_dir}/crtsh.txt" &
        
        wait
    } 
    
    # Combine results efficiently
    cat "${temp_dir}"/*.txt 2>/dev/null | grep -v '^$' | sort -u > "${output_dir}/all_subdomains.txt"
    rm -rf "$temp_dir"
}

# Optimized JavaScript analysis using aria2c
fetch_js_files() {
    local domain="$1"
    local output_dir="${OUTPUT_DIR}/${domain}"
    ...
    grep -E '\.js$' "${output_dir}/endpoints/wayback_urls.txt" | sort -u > "${js_dir}/js_files.txt"
    ...
    # Process JS files in parallel using GNU parallel
...
extract_params() {
    ...
    grep -aoP '(?<=[\?&])[^=&]+(?=\=)' "${output_dir}/endpoints/wayback_urls.txt" | \
...
# Main function optimized for parallel execution
main() {
